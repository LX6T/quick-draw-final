 ...
 * Code partially adapted from https://github.com/deepjavalibrary/djl-demo.
 */
public class DoodlePrediction {
	 ...
	 *                            of the DL model.
	 */
	public static void main(final String[] args) throws ModelException, IOException, TranslateException {
		if (args.length != 2) {
		...
		printPredictions(new DoodlePrediction().getPredictions(new File(args[0]), Integer.parseInt(args[1])));
	}
	 ...
	 */
	public static StringBuilder printPredictions(final List<Classifications.Classification> predictions) {
		...
		for (final Classifications.Classification classification : predictions) {
			sb.append("TOP ").append(i).append(" : ").append(classification.getClassName()).append(" : ")
	...
	public static StringBuilder givePredictions(final List<Classifications.Classification> predictions) {
		final StringBuilder sb = new StringBuilder();
	...
	public DoodlePrediction() throws ModelException, IOException {
		final ImageClassificationTranslator translator = ImageClassificationTranslator.builder()
				.addTransform(new ToTensor()).optFlag(Image.Flag.GRAYSCALE).optApplySoftmax(true).build();
		...
		final Criteria<Image, Classifications> criteria = Criteria.builder()
				...
				.optModelUrls("src/main/resources/ml/doodle_mobilenet.zip").optOption("mapLocation", "true")
				.optTranslator(translator).build();
	...
	public List<Classifications.Classification> getPredictions(BufferedImage bufImg, final int k)
		...
		bufImg = Scalr.resize(bufImg, Scalr.Method.SPEED, Scalr.Mode.FIT_TO_WIDTH, 65, 65, Scalr.OP_ANTIALIAS);

		final Classifications classifications = model.newPredictor()
				.predict(new BufferedImageFactory().fromImage(bufImg));
